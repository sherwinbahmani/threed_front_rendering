...
def main(args):
    scenes = sorted(os.listdir(args.path_in))
    for scene in tqdm(scenes):
        # if "01492580-b04d-4865-932d-c6fc225ba3b8_LivingDiningRoom-34353" not in scene:
        if "023fcd68-dd38-420c-a518-87e82b80ac1e_LivingDiningRoom-87566" not in scene:
        # if "0af3b388-5d81-4b78-b7f0-c19786f9f9c1_LivingRoom-9812" not in scene:
        # if "0aad3aa3-ec12-49a0-b7cf-548d42b0b12b_LivingDiningRoom-98003" not in scene:
        # if "00ecd021-64bb-4bc4-b754-1796d2a12965_LivingDiningRoom-84752" not in scene:
        # if "0aa05d5a-81d5-497b-832c-c90c3fe73a36_LivingDiningRoom-1171" not in scene:
        # if "00ad8345-45e0-45b3-867d-4a3c88c2517a_LivingRoom-46201" not in scene:
        # if "0c2cf150-5293-43d7-acbf-8c1b0a67070a_LivingDiningRoom-4261" not in scene:
        # if "0e566be7-4271-47be-9f22-65747d517ed9_LivingDiningRoom-11176" not in scene:
            continue
        ...
        #     continue
        set_seed(args.seed)
        labels = load_labels(path_in_labels, args.max_coords)
        ...
        scene_grid, bed_grid, room_layout, bed_center, valid_mask = create_voxel_grid(**labels)
        camera_coords, target_coords = sample_camera_positions(scene_grid, bed_grid, room_layout, bed_center, args.max_coords, args.num_samples_scene)
        if camera_coords is None or target_coords is None:
            print(f"{scene} does not have the bed or floorplan in the center")
            continue

        if True: #target_coords.shape[0] == args.num_samples_scene and camera_coords.shape[0] == args.num_samples_scene:
...
def sample_camera_positions(scene_grid, bed_grid, room_layout, bed_center, max_coords, num_samples_scene=10,
                            camera_height=1.7, seed=0, top_perc=0.5, dist_perc=0.0):
    ...
    # print("bed center", torch.stack(torch.where(bed_grid), dim=1).float().mean(0))
    ...
    # print("valid_indices", valid_indices.shape, valid_indices, valid_indices.amin(0), valid_indices.amax(0))
    ...
    valid_indices = valid_indices[valid_ind_sorted]
    ...
    valid_distance_ind = torch.where(torch.logical_and(torch.logical_and(torch.logical_and(l1_dist[:, 0] >= 0.4, l1_dist[:, 0] < 10.0), l1_dist[:, 2] >= 0.4), l1_dist[:, 2] < 10.0))[0]
    # print("l1_dist", l1_dist.shape, l1_dist.min(), l1_dist.max())
    # print("l1_dist fil", l1_dist[valid_distance_ind[:5]], l1_dist[valid_distance_ind[-5:]])
    # print("indices_combined", indices_combined.shape)
    # print("valid_distance_ind", valid_distance_ind.shape)
    i=-1
    valid_distance_ind_chosen = valid_distance_ind[i]
    indices_chosen = indices_combined[valid_distance_ind_chosen]
    # print("indices_combined[0]", indices_combined[0], l1_dist[indices_chosen[0]], l1_dist[indices_chosen[1]])
    if valid_distance_ind.shape[0] == 0:
        print("Could not find a valid distance inside floorplan")
        return None, None
    print(valid_layout.shape, torch.where(valid_layout))
    # coords_list = [((155, 75), (155, 50)), ((155, 50), (100, 50))] #01492580-b04d-4865-932d-c6fc225ba3b8_LivingDiningRoom-34353
    coords_list = [((42, 100), (130, 100))] #023fcd68-dd38-420c-a518-87e82b80ac1e_LivingDiningRoom-87566
    # coords_list = [((90, 80), (90, 100)), ((90, 100), (140, 100)), ((140, 100), (140, 80))] #0af3b388-5d81-4b78-b7f0-c19786f9f9c1_LivingRoom-9812
    # coords_list = [((130, 40), (130, 140))] #0aad3aa3-ec12-49a0-b7cf-548d42b0b12b_LivingDiningRoom-98003
    # coords_list = [((150, 150), (150, 85)), ((150, 85), (137, 90))] #00ecd021-64bb-4bc4-b754-1796d2a12965_LivingDiningRoom-84752
    # coords_list = [((190, 150), (190, 110)), ((190, 110), (150, 110))] #0aa05d5a-81d5-497b-832c-c90c3fe73a36_LivingDiningRoom-1171
    # coords_list = [((140, 42), (110, 42)), ((110, 42), (110, 95))] #00ad8345-45e0-45b3-867d-4a3c88c2517a_LivingRoom-46201
    # coords_list = [((60, 110), (60, 120)), ((60, 120), (130, 120))] #0c2cf150-5293-43d7-acbf-8c1b0a67070a_LivingDiningRoom-4261
    # coords_list = [((40, 110), (40, 155)), ((40, 155), (100, 155))] #0e566be7-4271-47be-9f22-65747d517ed9_LivingDiningRoom-11176
    # assert False
    path_astar = []
    for start_coords, end_coords in coords_list:
        path_astar_sub = []
        # while len(path_astar_sub) < num_samples_scene:
        in_valid_layout = (valid_layout == False).cpu().numpy().astype(int)
        path_astar_sub = astar(in_valid_layout, start_coords, end_coords)
        print(len(path_astar_sub))
        path_astar += path_astar_sub
    if path_astar == []:
    ...
    # valid_coords_selected = valid_coords[:num_samples_scene_new]
    target_coords_selected = target_coords_selected.repeat(valid_coords_selected.shape[0], 1)
    num_samples_scene = valid_coords_selected.shape[0]
...
def load_labels(path_file, max_coords, device='cuda'):
...
def create_voxel_grid(class_labels, translations, sizes, angles, room_layout, floor_plan_centroid, bed_size=1.0, dims=(256, 256, 256), num_vertices = 256):
    """
    ...
    # # Pick random object
    # bed_index = obj_indices[torch.randperm(obj_indices.shape[0])[0]]
    # Pick largest object
    bed_index = obj_indices[0]
    bed_grid = None
    bed_center = None
    valid_mask = []
...
if __name__ == '__main__':
    ...
    parser.add_argument(
            "--path-in",
            default="/path/to/data/3dfront/processed/living_room_without_lamps/",
            help="Path to scenes with bounding boxes"
        )
    parser.add_argument(
            "--path-out",
            default="/path/to/data/3dfront/processed/test/labels_living",
            help="Path to scenes with bounding boxes"
        )
    parser.add_argument(
            "--max-coords",
            default=[12.0, 4.0, 12.0],
