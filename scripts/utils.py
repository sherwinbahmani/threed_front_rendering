...
def get_textured_trimesh(mesh, textures_folder):
    ...
    # We assume that textures_folder contains subfolders with image textures.
    texture = np.random.choice(textures_folder)
    if os.path.isfile(os.path.join(texture, "texture.png")):
        texture_image = os.path.join(texture, "texture.png")
    elif os.path.isfile(os.path.join(texture, "texture.jpg")):
        texture_image = os.path.join(texture, "texture.jpg")
    else:
        raise NotImplementedError()

    textured_mesh = TexturedMesh.from_faces(
        vertices=vertices,
        uv=uv,
        faces=faces,
        material=Material.with_texture_image(texture_image)
    )

    tr_mesh = trimesh.Trimesh(
...
def get_scene_element(scene, vertices, faces, textures_folder):
    """Given the vertices and the faces of any scene element e.g. floor, walls
    etc, create a trimesh and a simple-3dviz TexturedMesh.
    """
...
def get_floor_plan(scene, floor_textures):
    ...
    return get_scene_element(scene, vertices, faces, floor_textures)


def get_walls(scene, wall_textures):
    """Return the walls of the scene as a trimesh mesh and a simple-3dviz
    TexturedMesh."""
    vertices, faces = scene.walls
    return get_scene_element(scene, vertices, faces, wall_textures)

def get_windows(scene, window_textures):
    """Return the windows of the scene as a trimesh mesh and a simple-3dviz
    TexturedMesh."""
    vertices, faces = scene.windows
    return get_scene_element(scene, vertices, faces, window_textures)

def get_doors(scene, door_textures):
    """Return the doors of the scene as a trimesh mesh and a simple-3dviz
    TexturedMesh."""
    vertices, faces = scene.doors
    return get_scene_element(scene, vertices, faces, door_textures)
...
def export_scene(output_directory, trimesh_meshes, names=None):
    ...
    # Get a mesh for the objects and the floor
    # Exclude the last mesh that corresponds to the walls
    combined = trimesh.util.concatenate(trimesh_meshes[:-1])
    obj_out = trimesh.exchange.obj.export_obj(combined)
    with open(os.path.join(output_directory, "scene.obj"), "w") as f:
        f.write(obj_out)

    # Get a mesh for the objects
    # Exclude the last two meshes that correspond to the floor and walls
    combined = trimesh.util.concatenate(trimesh_meshes[:-2])
    obj_out = trimesh.exchange.obj.export_obj(combined)
    with open(os.path.join(output_directory, "scene_objects.obj"), "w") as f:
        f.write(obj_out)
