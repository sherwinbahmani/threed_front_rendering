...
def main(args):
    ...
    for scene in tqdm(scenes):
        ...
        if not os.path.isfile(path_in_labels) or os.path.isfile(path_out_labels):
            continue
        labels, bed_index = load_labels(path_in_labels, args.max_coords, beds_only=args.beds_only)
        scene_grid, bed_grid, room_layout, bed_center = create_voxel_grid(**labels, beds_only=args.beds_only)
        ...
        if camera_coords is None or target_coords is None:
            print(f"{scene} does not have the bed in the center")
            continue
        ...
        if target_coords.shape[0] == args.num_samples_scene and camera_coords.shape[0] == args.num_samples_scene:
            ...
            if args.beds_only:
                for k in ["class_labels", "translations", "sizes", "angles"]:
                    labels_out[k] = labels_out[k][[bed_index.cpu().item()],:]
                # Center translations to zero
                labels_out['translations'][:,[0,2]] = 0
                # Set whole floorplan to valid
                room_layout[:] = 255
...
def sample_camera_positions(scene_grid, bed_grid, room_layout, bed_center, max_coords, num_samples_scene=10,
    ...
    if beds_only:
        bed_center_calc = torch.stack(torch.where(bed_grid)).float().mean(1)
        is_bed_center = torch.allclose(bed_center_calc[0], torch.tensor(bed_grid.shape[0] - 1, device=bed_center_calc.device)/2, 0.1) and torch.allclose(bed_center_calc[2], torch.tensor(bed_grid.shape[2] - 1, device=bed_center_calc.device)/2, 0.1)
        if not is_bed_center:
            return None, None
    ...
    valid_layout = scene_layout_mask
    ...
    bed_center_calc_flipped = torch.stack(torch.where(bed_grid_flipped)).float().mean(1)
    target_indices = bed_center_calc_flipped.unsqueeze(0) # Use bed center 
    ...
    target_coords_selected = target_coords.repeat(num_samples_scene, 1)
...
def load_labels(path_file, max_coords, device='cuda', beds_only=False):
    ...
    if beds_only:
        ...
        class_labels = class_labels[[bed_index],:]
        translations = translations[[bed_index],:]
        sizes = sizes[[bed_index],:]
        angles = angles[[bed_index],:]
        # Set center to 0,0
        translations[:,[0, 2]] = 0
    else:
        bed_index = None
    ...
    return labels, bed_index

def create_voxel_grid(class_labels, translations, sizes, angles, room_layout, beds_only=False, dims=(256, 256, 256), num_vertices = 256):
    """
    ...
    return voxel_masks, bed_grid, room_layout, bed_center
...
if __name__ == '__main__':
    ...
    parser.add_argument(
            "--path-out",
            default="/path/to/data/3dfront/processed/bedrooms_without_lamps_single_beds_center_no_floorplan_3/labels",
            help="Path to scenes with bounding boxes"
        )
    parser.add_argument(
            "--max-coords",
